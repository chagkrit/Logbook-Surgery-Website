...
export default function LoginPage({ onLogin, onActivate, onRequestReset, initialMessage = "" }) {
  ...
  const [classYear, setClassYear] = useState(String(defaultStartingClassYear));
  const [cohortYear, setCohortYear] = useState(String(defaultAcademicYear));
  ...
  async function submit(event) {
    ...
    try {
      if (mode === "reset") {
        ...
        const nextMessage = await onActivate({ role, email, password, fullName, studentCode, studentGroup, classYear: Number(classYear), cohortYear: Number(cohortYear) });
      ...
      <section className="login-panel" aria-labelledby="login-title">
        ...
        <form onSubmit={submit}>
          ...
          {mode === "activate" && role === "student" && <>
            ...
            <label>ชั้นปีที่เริ่มใช้งาน<div className="input-wrap"><UserIcon size={20} /><select value={classYear} onChange={(event) => setClassYear(event.target.value)} required><option value="5">นักศึกษาแพทย์ชั้นปีที่ 5</option><option value="4">นักศึกษาแพทย์ชั้นปีที่ 4</option></select></div></label>
            <label>ปีการศึกษา<div className="input-wrap"><UserIcon size={20} /><input type="number" min="2500" max="2700" value={cohortYear} onChange={(event) => setCohortYear(event.target.value)} placeholder="2569" required /></div></label>
          </>}
