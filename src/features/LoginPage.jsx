...
export default function LoginPage({ onLogin, onActivate, onRequestReset, initialMessage = "" }) {
  ...
  const invitedRole = registerRole === "staff" || registerRole === "student" ? registerRole : null;
  ...
  async function submit(event) {
    ...
    try {
      if (mode === "reset") {
        ...
        const nextMessage = await onActivate({ role, email, password, fullName, studentCode });
    ...
    <main className="login-page">
      ...
      <section className="login-panel" aria-labelledby="login-title">
        ...
        <form onSubmit={submit}>
            ...
            <div className="role-selector">
              {["student", "staff"].map((item) => (
                <button type="button" key={item} className={role === item ? "active" : ""} onClick={() => { setRole(item); setError(""); }}>
                  <UserIcon size={22} /> {item === "staff" ? "Staff" : "Student"}
                </button>
              ))}
            </div>
          </fieldset>}
          ...
          <p className="login-help">Student ลงทะเบียนด้วยอีเมลของตนเองได้และต้องยืนยันอีเมลก่อนใช้งาน ส่วนบัญชี Staff ต้องอยู่ในรายชื่อที่ภาควิชาอนุมัติ</p>
        </form>
      </section>
    </main>
  );
